...
void lastline(int n1, int n2, int n3) {
        ...
        if (strcmp(temp, ":: ") == 0) {
            ...
            for (i = 0, flag2 = cnt4 = 0, doc = -1; i < len; ++i) {
                if (temp[i] != ' ') {
                    if (doc == -1) doc = i;
                    if (isupper(temp[i])) {
                        mes[cnt3].character[cnt4++] = temp[i] = tolower(temp[i]);
                    } else {
                        mes[cnt3].character[cnt4++] = temp[i];
                    }
                    flag2 = 1;
                } else if (flag2) {
                    mes[cnt3].rank = cnt2;
